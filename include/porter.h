...
static char s[500]={0};         /* a char * (=string) pointer; passed into b above */

//#define INC 50           /* size units in which s is increased */
//static int i_max = INC;  /* maximum offset in s */

//static char *s=(char *) calloc (i_max, sizeof(char));         /* a char * (=string) pointer; passed into b above */
...
char * stemword(const char * str) {
	int i;
	for (i = 0; str[i]; i++) {
		//if (i == i_max) increase_s();

		if UC(str[i]) s[i] = FORCELC(str[i]);
